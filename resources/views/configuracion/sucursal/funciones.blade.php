...
//----------------------GENERAR TABLA-------------------------
	...
	function generarTabla(page, filtro) {
		$.ajax({
			...
			url: '{{ url('configuracion/sucursal') }}',
			type: 'GET',
...
//----------------MOSTRAR MODAL ACTUALIZAR------------------
	function mostrar(id) {
		$.ajax({
			headers: {'X-CSRF-TOKEN':'{{ csrf_token() }}'},
		 	url: '{{url('configuracion/sucursal')}}/'+id+'/edit',
		 	type: 'GET',
		 	dataType: 'json',
		 	beforeSend: function () {
		 		$('.box').append('<div class="overlay">'+
...
//-----------------ACTUALIZAR DATOS-------------------------
	$('#actualizar').submit(function (e) {
		...
		$.ajax({
			headers: {'X-CSRF-TOKEN':'{{ csrf_token() }}'},
			url: '{{url('configuracion/sucursal')}}/'+$('#id').val(),
			type: 'PUT',
			data: datos,
			dataType: 'json',
			beforeSend: function () {
				$('#actualizar .form-control').prop('disabled', true);
				$('#actualizarIngreso').prop('disabled', true);
		 		$('#actualizarIngreso').html('<i class="fa fa-refresh fa-spin"></i>');
		 		$('#mostrarModal .cerrar').removeAttr('data-dismiss');
			},
			success: function (data) {
				$('#actualizar .form-control').prop('disabled', false);
		 		$('#actualizarIngreso').prop('disabled', false);
		 		$('#actualizarIngreso').html('Actualizar');
		 		$('#mostrarModal .cerrar').attr('data-dismiss','modal');
...
//-------------------ELIMINAR DATOS------------------------
	$('#eliminar').submit(function (e) {
		...
		$.ajax({
			headers: {'X-CSRF-TOKEN':'{{ csrf_token() }}'},
			url: '{{url('configuracion/sucursal')}}/'+$('#eliminarId').val(),
			type: 'DELETE',
			dataType: 'json',
			beforeSend: function () {

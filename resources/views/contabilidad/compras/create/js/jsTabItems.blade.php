...
$('#btnGuardarItems').click(function () {
	...
	if (esValidoIngresarItem()) {
		...
		precio[contItems] = $('#precioProducto').val();
		subtotalCompra[contItems] = cantProducto[contItems] * precio[contItems];
		...
		var fila = '<tr id="filaItem'+contItems+'">'+
						...
						'<td>$'+precio[contItems]+'</td>'+
						'<td>'+cantProducto[contItems]+'</td>'+
						'<td>$'+subtotalCompra[contItems].toFixed(2)+'</td>'+
					'</tr>';
		calculoCompra();
		calculoRetencionIva();
...
function calculoCompra() {
	var subtotal = subtotalCompra.reduce(sumar);
	var montoNoFac = $('#montoSinIva').val();
	var montoFac = subtotal - montoNoFac;
	var iva = montoFac * parseFloat('{{ $empresa->iva }}') / 100;
	var retIVA = $('#retencionIVA').val();
	var retRenta = $('#retencionRenta').val();
	var total = subtotal + iva - retIVA - retRenta;

	$('#iva').val(iva.toFixed(2));
	$('#montoConIva').val(montoFac.toFixed(2));
